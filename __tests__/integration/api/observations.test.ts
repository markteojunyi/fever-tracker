...
describe("GET /api/observations", () => {
  ...
  it("returns an empty array when the child has no observations", async () => {
    const body = await (
      await GET(get(`/api/observations?childId=${childId}`))
    ).json();
    expect(body).toEqual([]);
  });
  ...
  it("returns observations sorted by observedAt descending (most recent first)", async () => {
    await Observation.create({
      ...
      observedAt: new Date("2025-01-16T08:00:00Z"),
    });
    await Observation.create({
      childId,
      content: "Looks tired",
      observedAt: new Date("2025-01-16T14:00:00Z"),
    });
    await Observation.create({
      childId,
      content: "Cough worsening",
      observedAt: new Date("2025-01-16T20:00:00Z"),
    });
  ...
  it("does not return observations belonging to a different child", async () => {
    const other = await Child.create({
      name: "Other",
      dateOfBirth: new Date("2020-01-01"),
    });
    await Observation.create({
      childId: other._id,
      content: "Other child obs",
    });
...
describe("POST /api/observations", () => {
  it("creates an observation and returns 201", async () => {
    const res = await POST(
      post("/api/observations", { childId, content: "Looks drowsy" })
    );
  ...
  it("persists the observation to the database", async () => {
    await POST(
      post("/api/observations", { childId, content: "Refused to eat" })
    );
  ...
  it("trims whitespace from content", async () => {
    const res = await POST(
      post("/api/observations", { childId, content: "  Had a nap  " })
    );
  ...
  it("returns 400 when content is only whitespace", async () => {
    const res = await POST(
      post("/api/observations", { childId, content: "   " })
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 404 when the child does not exist", async () => {
    const fakeId = new mongoose.Types.ObjectId();
    const res = await POST(
      post("/api/observations", { childId: fakeId, content: "Ghost" })
    );
    expect(res.status).toBe(404);
  });
});
