...
describe("GET /api/medication-logs", () => {
  it("returns an empty array when no logs exist", async () => {
    const body = await (
      await GET(get(`/api/medication-logs?childId=${childId}`))
    ).json();
    expect(body).toEqual([]);
  });
  ...
  it("returns logs for the specified child sorted by administeredAt descending", async () => {
    await MedicationLog.create({
      ...logPayload(),
      administeredAt: new Date("2025-01-16T08:00:00Z"),
    });
    await MedicationLog.create({
      ...logPayload(),
      administeredAt: new Date("2025-01-16T14:00:00Z"),
    });
    ...
    // Most recent first
    expect(
      new Date(body[0].administeredAt) > new Date(body[1].administeredAt)
    ).toBe(true);
  });
  ...
  it("filters logs by date when the date param is provided", async () => {
    await MedicationLog.create({
      ...logPayload(),
      administeredAt: new Date("2025-01-16T10:00:00"),
    });
    await MedicationLog.create({
      ...logPayload(),
      administeredAt: new Date("2025-01-17T10:00:00"),
    });
...
describe("POST /api/medication-logs", () => {
  ...
  it("persists the log to the database", async () => {
    await POST(
      post("/api/medication-logs", logPayload({ administeredBy: "Dad" }))
    );
  ...
  it("saves optional notes when provided", async () => {
    const res = await POST(
      post("/api/medication-logs", logPayload({ notes: "With food" }))
    );
