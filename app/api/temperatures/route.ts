...
export async function GET(request: NextRequest) {
  try {
    ...
    const readings = await TemperatureReading.find({ childId })
      .sort({ timestamp: 1 });

    return NextResponse.json(readings);
  } catch (error) {
    return NextResponse.json({ error: 'Failed to fetch temperatures' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to create temperature reading' }, { status: 500 });
  }
}